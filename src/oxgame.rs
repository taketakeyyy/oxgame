...
pub fn make_initialized_grid() -> Vec<Vec<Masu>> {
    let mut grid = vec![];
...
fn remove_stone(grid: &mut Vec<Vec<Masu>>, h: usize, w: usize) {
    grid[h][w].masu = Masu::EMPTY;
}
...
pub fn run_ai_strategy(grid: &mut Vec<Vec<Masu>>, who_turn: i32) -> Node {
    return minimax(grid, who_turn, 0, 99, 99);  // 99はダミー
}
